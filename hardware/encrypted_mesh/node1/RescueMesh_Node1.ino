...
// - SOS sender
// - SOS relay
// - Receives NODE_B SOS and relays it toward RESCUE
...
static void sendAck(
    ...
    if (!hexToBytes(OWN_ED25519_PRIVATE, privateKey, 32)) {
        Serial.println("ERROR: invalid Ed25519 private key");
        return;
    }

    if (!hexToBytes(OWN_ED25519_PUBLIC, publicKey, 32)) {
        Serial.println("ERROR: invalid Ed25519 public key");
        return;
    }

    uint8_t message[sizeof(AckPacket) - SIGNATURE_LEN];
...
static bool createSOS(
    ...
    // -------------------------------------------------------------------------
    // Derive encryption key using Rescue X25519 public key
    // -------------------------------------------------------------------------
    ...
    if (!encryptChaChaPoly(
        ...
        Serial.println("ERROR: ChaCha20-Poly1305 encryption failed");
        return false;
    }
...
static void sendSOS() {
    ...
    for (int attempt = 1; attempt <= ORIGIN_RETRIES; attempt++) {
        ...
        if (waitForAck(packet.origin_root)) {
            ...
            Serial.println("================================");
            Serial.println("SOS DELIVERED / ACK RECEIVED");
            Serial.println("================================");
    ...
    Serial.println("SOS delivery failed after retries");
...
static bool verifyDataPacket(
    ...
    if (!verifyEd25519(
        ...
        Serial.println("DATA signature verification failed");
        return false;
    }
...
static bool competingPacketHeard(
    ...
    if (packetSize == sizeof(MeshPacket)) {
        ...
        if (packet.magic == PROTOCOL_MAGIC &&
            ...
            // If the packet already contains our bit, it means
            // it has already passed through this node.
...
static void relayPacket(
    ...
    // -------------------------------------------------------------------------
    // Stronger RSSI -> shorter delay
    // Weaker RSSI -> longer delay
    // -------------------------------------------------------------------------
    ...
    while (millis() - start < relayDelay) {
        ...
        if (competingPacketHeard(packet.origin_root)) {
    ...
    // -------------------------------------------------------------------------
    // Update routing state EXACTLY ONCE
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Send unchanged encrypted payload/signature
    // Routing fields are intentionally mutable.
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // ACK the origin after successful relay
    // -------------------------------------------------------------------------
...
static void receivePackets() {
    ...
    if (!verifyDataPacket(packet)) {
        Serial.println("Rejected invalid DATA packet");
        return;
    }

    // -------------------------------------------------------------------------
    // If our bit is already set, do not relay again.
    // -------------------------------------------------------------------------
